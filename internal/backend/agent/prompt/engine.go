// engine.go 实现 MVP 阶段 prompt 编译器与工具过滤逻辑。
...
import (
	...
	promptassets "cursor/internal/prompt"
)
...
type CompileInput struct {
	...
	// RequestContext 表示本轮用户动作携带
// request_context
	RequestContext *agentv1.RequestContext
	// PendingAssistantOutputs 表示尚未收口
// assistant 输出记录
...
// buildMessagesFromStructuredState 
// summary / plan / todos 编译成下一轮模型可消费的消息
...
// buildMessagesFromRequestContext 
// request_context 中的用户环境信息编译成上游可消费消息
...
// buildRequestContextUserInfoSection 构
// <user_info> 片段
...
// buildRequestContextAgentTranscriptsSection 构
// <agent_transcripts> 片段
...
// buildRequestContextAgentSkillsSection 构
// <agent_skills> 片段
...
// buildRequestContextRulesSection 构
// <rules> 片段
...
// buildRequestContextMCPFileSystemSection 构
// <mcp_file_system> 片段
...
// buildMessagesFromPendingAssistantOutputs 
// `pending_tool_calls` 中的原始记录转换为安全的 assistant 文本消息
...
// buildMessagesFromCommittedTurns 把已提交
// `ConversationStateStructure.turns` 映射为有
// prompt 消息
...
// buildMessagesFromConversationStep 把单
// ConversationStep 映射
// prompt 消息
...
// sanitizePromptAsset 去除资产文件中的文档性标题，只保留真
// prompt 文本
...
// decodeToolsFromBaseline 从原始工
// JSON 中按原始顺序解码工具列表与名称
//
// 当前约束
// 1. `tools.json` 就是
// mode 的工具暴露基线；
// 2. 必须保持原始顺序与原始工具描述结构，不得
// prompt 编译阶段做二次过滤；
// 3. 未实现能力的显式错误语义必须
// runtime 负责，而不是由 prompt 编译器隐藏工具
...
// extractPendingAssistantText 从单
// pending assistant 原始记录中提取文本内容
