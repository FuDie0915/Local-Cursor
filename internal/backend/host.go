...
func NewHost(store *serverconfig.Store, certManager *certs.Manager) (*Host, error) {
	if store == nil {
	...
	host := &Host{
		...
		cliHTTPSListenAddr: cfg.CLIHTTPSListenAddr,
		tabServerBaseURL:  cfg.TabServerBaseURL,
		configs:           configs,
...
// CLIHTTPSListenAddr 返回 CLI HTTPS 监听地址。
func (host *Host) CLIHTTPSListenAddr() string {
	if host == nil {
		return ""
	}
	host.runMu.RLock()
	defer host.runMu.RUnlock()
	return host.cliHTTPSListenAddr
}

// CLIHTTPSRunning 返回 CLI HTTPS 监听器是否在运行。
func (host *Host) CLIHTTPSRunning() bool {
	if host == nil {
		return false
	}
	host.runMu.RLock()
	defer host.runMu.RUnlock()
	return host.httpsServer != nil
}
...
func (host *Host) Start() error {
	...
	// 启动 CLI HTTPS+HTTP/2 监听器
	if err := host.startHTTPSLocked(cfg); err != nil {
		// HTTPS 失败不阻断 HTTP，仅记录
		logger.Errorf("启动 CLI HTTPS 监听器失败: %v", err)
	}

	return nil
}

// startHTTPSLocked 启动 CLI HTTPS+HTTP/2 监听器。
// Go http.Server.ServeTLS 自动启用 HTTP/2 ALPN，完美兼容 connect-node 的 HTTP/2 传输。
// 证书由 certManager 为请求的 SNI 动态签发。
// 必须在 host.runMu.Lock() 下调用。
func (host *Host) startHTTPSLocked(cfg serverconfig.Config) error {
	if host.certManager == nil {
		logger.Infof("CLI HTTPS 监听器跳过: certManager 未配置")
		return nil
	}
	addr := host.cliHTTPSListenAddr
	...
	tlsConfig := &tls.Config{
		GetCertificate: func(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
			...
			return host.certManager.CertificateForServerName(serverName)
		},
	...
	if err != nil {
		return fmt.Errorf("监听 CLI HTTPS %s 失败: %w", addr, err)
	}
	host.cliHTTPSListenAddr = ln.Addr().String()
	host.httpsServer = httpsServer

	go func(srv *http.Server, listener net.Listener) {
		logger.Infof("CLI HTTPS 监听器开始提供服务 listen_addr=%s", listener.Addr().String())
		if err := srv.ServeTLS(listener, "", ""); err != nil && err != http.ErrServerClosed {
			logger.Errorf("CLI HTTPS 监听器异常退出: %v", err)
			host.runMu.Lock()
			if host.httpsServer == srv {
				host.httpsServer = nil
			}
			host.runMu.Unlock()
		}
	}(httpsServer, ln)

	logger.Infof("CLI HTTPS 监听器启动成功 listen_addr=%s", host.cliHTTPSListenAddr)
	return nil
}
...
func (host *Host) Stop(ctx context.Context) error {
	...
	if httpsServer != nil {
		if err := httpsServer.Shutdown(ctx); err != nil {
			errs = append(errs, fmt.Errorf("stop CLI HTTPS: %w", err))
		}
	}
	if serverInstance := httpServer; serverInstance != nil {
...
func (host *Host) rebuildLocked(cfg serverconfig.Config) error {
	...
	host.tabServerBaseURL = cfg.TabServerBaseURL
	...
	host.mux = server.New(
		...
		// CLI 版 cursor-agent 通过独立的 ServerConfigService 获取服务器配置。
		// 复用 AiService/GetServerConfig 的 mock builder。
		server.POST("/aiserver.v1.ServerConfigService/GetServerConfig",
			server.Name("server_config_service"),
			server.ConnectUnary(),
			server.Local(upstream.MockProtoAction(routeDeps, upstream.CompatRouteConfig{
				Name:          "server_config_service",
				StatusCode:    http.StatusOK,
				MockProtoType: "aiserver.v1.GetServerConfigResponse",
				MockBuilder:   upstream.ServerConfigMockBuilder,
			})),
		...
		// CLI 版 cursor-agent (2026.07+) 通过 AiService 获取可用模型列表和默认模型，
		// 而非旧版 AgentService。两条路由使用相同的 mock builder。
		server.POST("/aiserver.v1.AiService/GetUsableModels",
			server.Name("ai_service_usable_models"),
			server.ConnectUnary(),
			server.Local(upstream.MockProtoAction(routeDeps, upstream.CompatRouteConfig{
				Name:          "ai_service_usable_models",
				StatusCode:    http.StatusOK,
				MockProtoType: "agent.v1.GetUsableModelsResponse",
				MockBuilder:   upstream.AgentUsableModelsMockBuilder,
			})),
			server.Upstream(upstream.DirectAction(routeDeps, upstream.CompatRouteConfig{
				Name: "ai_service_usable_models",
			})),
		...
		server.POST("/aiserver.v1.AiService/GetDefaultModelForCli",
			server.Name("ai_service_default_model_for_cli"),
			server.ConnectUnary(),
			server.Local(upstream.MockProtoAction(routeDeps, upstream.CompatRouteConfig{
				Name:          "ai_service_default_model_for_cli",
				StatusCode:    http.StatusOK,
				MockProtoType: "agent.v1.GetDefaultModelForCliResponse",
				MockBuilder:   upstream.AgentDefaultModelForCliMockBuilder,
			})),
			server.Upstream(upstream.DirectAction(routeDeps, upstream.CompatRouteConfig{
				Name: "ai_service_default_model_for_cli",
			})),
		),
		// CLI 版 cursor-agent 通过 AgentService 获取可用模型列表和默认模型。
		server.POST("/agent.v1.AgentService/GetUsableModels",
			server.Name("agent_usable_models"),
			server.ConnectUnary(),
			server.Local(upstream.MockProtoAction(routeDeps, upstream.CompatRouteConfig{
				Name:          "agent_usable_models",
				StatusCode:    http.StatusOK,
				MockProtoType: "agent.v1.GetUsableModelsResponse",
				MockBuilder:   upstream.AgentUsableModelsMockBuilder,
			})),
		...
		server.POST("/agent.v1.AgentService/GetDefaultModelForCli",
			server.Name("agent_default_model_for_cli"),
			server.ConnectUnary(),
			server.Local(upstream.MockProtoAction(routeDeps, upstream.CompatRouteConfig{
				Name:          "agent_default_model_for_cli",
				StatusCode:    http.StatusOK,
				MockProtoType: "agent.v1.GetDefaultModelForCliResponse",
				MockBuilder:   upstream.AgentDefaultModelForCliMockBuilder,
			})),
		...
		tabServerUpstreamProcedure("/aiserver.v1.AiService/StreamCpp", "ai_stream_cpp", server.ConnectStream(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/StreamNextCursorPrediction", "ai_stream_next_cursor_prediction", server.ConnectStream(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/GetCppEditClassification", "ai_get_cpp_edit_classification", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/RefreshTabContext", "ai_refresh_tab_context", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/CppConfig", "ai_cpp_config", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/CppEditHistoryStatus", "ai_cpp_edit_history_status", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/CppAppend", "ai_cpp_append", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/CppEditHistoryAppend", "ai_cpp_edit_history_append", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/ReportAiCodeChangeMetrics", "ai_report_ai_code_change_metrics", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/WriteGitCommitMessage", "ai_write_git_commit_message", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.AiService/WriteGitBranchName", "ai_write_git_branch_name", server.ConnectUnary(), routeDeps, host),
		...
		tabServerUpstreamProcedure("/aiserver.v1.CppService/AvailableModels", "cpp_available_models", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.CppService/RecordCppFate", "cpp_record_cpp_fate", server.ConnectUnary(), routeDeps, host),
		...
		tabServerUpstreamProcedure("/aiserver.v1.FileSyncService/FSSyncFile", "file_sync_sync_file", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.FileSyncService/FSIsEnabledForUser", "file_sync_is_enabled_for_user", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.FileSyncService/FSConfig", "file_sync_config", server.ConnectUnary(), routeDeps, host),
		tabServerUpstreamProcedure("/aiserver.v1.FileSyncService/FSUploadFile", "file_sync_upload_file", server.ConnectUnary(), routeDeps, host),
		...
		// tabServerUpstreamProcedure("/aiserver.v1.DashboardService/GetEffectiveUserPlugins", "dashboard_get_effective_user_plugins", server.ConnectUnary(), routeDeps, host),
		...
		// CLI 版 cursor-agent 通过 loginWithApiKey 调用此端点交换 token。
		// 返回 InjectAuthToken 使 CLI 跳过浏览器登录流程。
		server.POST("/auth/exchange_user_api_key",
			server.Name("auth_exchange_user_api_key"),
			server.HTTP(),
			server.Local(upstream.MockJSONAction(routeDeps, upstream.CompatRouteConfig{
				Name:       "auth_exchange_user_api_key",
				StatusCode: http.StatusOK,
				JSONBody: map[string]any{
...
func tabServerUpstreamProcedure(pattern string, name string, protocol server.RouteOption, deps upstream.Dependencies, host *Host) server.Option {
	direct := upstream.DirectAction(deps, upstream.CompatRouteConfig{Name: name})
	action := func(ctx *server.Context) error {
		if host == nil || host.tabServerBaseURL == "" {
			http.NotFound(ctx.Writer, ctx.Request)
			return nil
		}
		if ctx != nil && ctx.Request != nil && ctx.Request.URL != nil {
			baseURL, err := url.Parse(host.tabServerBaseURL)
			if err != nil {
