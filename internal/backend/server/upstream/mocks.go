...
func buildServerConfigPayload(*RequestContext) (map[string]any, error) {
	return map[string]any{
		...
		// cliSandboxDefaultEnabled=false 使 CLI 从服务端查询模型列表，
		// 而非使用沙箱内置默认值（沙箱模式下 CLI 不请求 GetUsableModels）。
		"cliSandboxDefaultEnabled": false,
		// http2Config=HTTP2_CONFIG_FORCE_ALL_DISABLED(=1) 强制 CLI agent transport
		// 使用 HTTP/1.1，使 goproxy MITM 能正常拦截和解析请求。
		// 枚举值对应 proto: HTTP2_CONFIG_FORCE_ALL_DISABLED = 1
		"http2Config": 1,
	}, nil
}
...
func buildAgentUsableModelsPayload(reqCtx *RequestContext) (map[string]any, error) {
	adapters, err := loadConfiguredModelAdapters(reqCtx)
	if err != nil {
		return nil, err
	}
...
func buildAgentDefaultModelForCliPayload(reqCtx *RequestContext) (map[string]any, error) {
	adapters, err := loadConfiguredModelAdapters(reqCtx)
	if err != nil {
		return nil, err
	}
