...
type ProxyState struct {
	...
	// CLIHTTPSListenAddr 表示 CLI 专用 HTTPS+HTTP/2 监听地址。
	CLIHTTPSListenAddr string `json:"cliHttpsListenAddr"`
	// CLIHTTPSRunning 表示 CLI 专用 HTTPS 监听器是否已启动。
	CLIHTTPSRunning bool `json:"cliHttpsRunning"`
...
func (s *ProxyService) GetState() ProxyState {
	...
	cliHTTPSListenAddr := ""
	cliHTTPSRunning := false
	if s.backendHost != nil {
		backendListenAddr = s.backendHost.ListenAddr()
		backendRunning = s.backendHost.IsRunning()
		cliHTTPSListenAddr = s.backendHost.CLIHTTPSListenAddr()
		cliHTTPSRunning = s.backendHost.CLIHTTPSRunning()
	}
	...
	return ProxyState{
		...
		CLIHTTPSListenAddr:    cliHTTPSListenAddr,
		CLIHTTPSRunning:       cliHTTPSRunning,
