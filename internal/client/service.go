...
type ProxyService struct {
	// proxy 表示 IDE 版 MITM 代理。
	proxy *mitm.ProxyServer
	// certManager 用于在代理监听地址变化时重建 MITM 服务，同时为 CLI HTTPS 监听器签发证书。
...
func NewProxyService(proxy *mitm.ProxyServer, certManager *certs.Manager, caCertPEM []byte) *ProxyService {
	...
	host, err := backend.NewHost(service.store, service.certManager)
	if err != nil {
...
func (s *ProxyService) ensureBackendHost() error {
	...
	host, err := backend.NewHost(s.store, s.certManager)
	if err != nil {
		return err
	}
	s.backendHost = host
