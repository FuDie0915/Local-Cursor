...
function normalizeConfig(source) {
  ...
  return {
    ...
    tabServerToken: asString(raw.tabServerToken),
  };
}
...
function buildConfigPayload(source = appState) {
  ...
  return {
    ...
    tabServerBaseURL: normalized.tabServerBaseURL,
    tabServerToken: normalized.tabServerToken,
  };
}
...
export async function saveTabServerConfig(baseURL, token) {
  const currentConfig = await loadPersistedUserConfig();
  const nextBaseURL = asString(baseURL);
  const nextToken = asString(token);
  appState.tabServerBaseURL = nextBaseURL;
  appState.tabServerToken = nextToken;
  return persistConfigPayload({
    ...currentConfig,
    tabServerBaseURL: nextBaseURL,
    tabServerToken: nextToken,
  });
}
